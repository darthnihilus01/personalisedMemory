...
export default function CaptureDemo({ onComplete }: CaptureDemoProps) {
  ...
  // STEP 1: typing completes -> phase = 'done'
  useEffect(() => {
  ...
  // STEP 2: done -> shift_up (speech card moves UP to make room)
  useEffect(() => {
  ...
  // STEP 3: shift_up -> synthesizing (branch line & badge appear)
  useEffect(() => {
    if (phase !== 'shift_up') return;
    const timer = setTimeout(() => setPhase('synthesizing'), 500);
    return () => clearTimeout(timer);
  }, [phase]);

  // STEP 4: synthesizing -> card_marcus (1st card: Marcus)
  useEffect(() => {
    if (phase !== 'synthesizing') return;
    const timer = setTimeout(() => setPhase('card_marcus'), 400);
    return () => clearTimeout(timer);
  }, [phase]);

  // STEP 5: card_marcus -> card_atlas (2nd card: Atlas)
  useEffect(() => {
    if (phase !== 'card_marcus') return;
    const timer = setTimeout(() => setPhase('card_atlas'), 380);
    return () => clearTimeout(timer);
  }, [phase]);

  // STEP 6: card_atlas -> card_inference (3rd card: System Inference)
  useEffect(() => {
    if (phase !== 'card_atlas') return;
    const timer = setTimeout(() => {
      setPhase('card_inference');
      if (onComplete) {
  ...
  const showSynthesizing = ['synthesizing', 'card_marcus', 'card_atlas', 'card_inference'].includes(phase);
  const showMarcus = ['card_marcus', 'card_atlas', 'card_inference'].includes(phase);
  const showAtlas = ['card_atlas', 'card_inference'].includes(phase);
  const showInference = phase === 'card_inference';

  // Smooth camera spring: typing zooms 1.06, done resets to 1.0, then shift_up moves UP y: -32
  const cameraVariants = {
    idle: { scale: 1, y: 0 },
    typing: { scale: 1.06, y: 0 },
    done: { scale: 1, y: 0 },
    shift_up: { scale: 1, y: -32 },
    synthesizing: { scale: 1, y: -32 },
    card_marcus: { scale: 1, y: -32 },
    card_atlas: { scale: 1, y: -32 },
    card_inference: { scale: 1, y: -32 },
  };
    ...
    <div ref={sectionRef} className="w-full py-24 md:py-32 px-6 relative z-20 overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center gap-6">
          ...
          <motion.div
            className="w-full max-w-[720px] bg-[#0d0e14] rounded-2xl flex flex-col relative border border-cyan-500/20 border-t-cyan-500/30 shadow-[0_0_35px_rgba(45,212,191,0.08)] p-6 md:p-8 gap-5 group"
          ...
          <AnimatePresence>
            {showSynthesizing && (
              ...
              >
                ...
                <div ref={contextRef} className="w-full flex flex-col gap-4 mt-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <AnimatePresence>
                      {showMarcus && (
                        <motion.div
                          className="bg-[#0b0d14] border border-white/[0.08] border-t-white/[0.14] rounded-2xl p-5 flex flex-col gap-3 shadow-[0_8px_24px_rgba(0,0,0,0.5)] hover:border-white/[0.18] transition-colors group"
                  ...
                  {/* Bottom Full-Width Row: System Inference (Card 3) */}
                  <AnimatePresence>
                    {showInference && (
                      <motion.div
